...
def rename_svg_glyph_names(data):
  ...
  svg_path = os.path.join(FONTS_FOLDER_PATH, 'ponticons.svg')
...
def generate_less(data):
  ...
  variables_file_path = os.path.join(LESS_FOLDER_PATH, '_ponticons-variables.less')
  icons_file_path = os.path.join(LESS_FOLDER_PATH, '_ponticons-icons.less')
  ...
  d.append('// --------------------------\n')
  d.append('@ponticons-font-path: "../fonts";')
  d.append('@ponticons-font-family: "%s";' % (font_name) )
  d.append('@ponticons-version: "%s";' % (font_version) )
  d.append('@ponticons-prefix: %s;' % (css_prefix) )
  d.append('')
  for ionicon in data['icons']:
    chr_code = ionicon['code'].replace('0x', '\\')
    d.append('@ponticon-var-%s: "%s";' % (ionicon['name'], chr_code) )
...
def generate_scss(data):
  ...
  font_version = data['version']
  css_prefix = data['prefix']
  variables_file_path = os.path.join(SCSS_FOLDER_PATH, '_ponticons-variables.scss')
  icons_file_path = os.path.join(SCSS_FOLDER_PATH, '_ponticons-icons.scss')

  d = []
  d.append('// PontIcons Variables')
  d.append('// --------------------------\n')
  d.append('$ponticons-font-path: "../fonts" !default;')
  d.append('$ponticons-font-family: "%s" !default;' % (font_name) )
  d.append('$ponticons-version: "%s" !default;' % (font_version) )
  d.append('$ponticons-prefix: %s !default;' % (css_prefix) )
  d.append('')
  for ionicon in data['icons']:
    chr_code = ionicon['code'].replace('0x', '\\')
    d.append('$ponticon-var-%s: "%s";' % (ionicon['name'], chr_code) )
...
def generate_css_from_scss(data):
  ...
  scss_file_path = os.path.join(SCSS_FOLDER_PATH, 'ponticons.scss')
  css_file_path = os.path.join(CSS_FOLDER_PATH, 'ponticons.css')
  css_min_file_path = os.path.join(CSS_FOLDER_PATH, 'ponticons.min.css')
...
def generate_html(data):
  ...
  template_path = os.path.join(BUILDER_PATH, 'cheatsheet', 'doc.html')
  group_path = os.path.join(BUILDER_PATH, 'cheatsheet', 'group.html')
  icon_path = os.path.join(BUILDER_PATH, 'cheatsheet', 'icon.html')
  ...
  f = open(group_path, 'r')
  section_html_template = f.read()
  f.close()

  f = open(icon_path, 'r')
  icon_html_template = f.read()
  f.close()
  ...
  for group in data['groups']:
    section_html = section_html_template
    section_html = section_html.replace('{{name}}', group['name'])
    section_content = []
    for icon in group['icons']:
        icon_html = icon_html_template
        icon_html = icon_html.replace('{{name}}', icon['name'])
        icon_html = icon_html.replace('{{prefix}}', data['prefix'])
        icon_html = icon_html.replace('{{code}}', icon['code'].replace('0x', '').upper())
        section_content.append(icon_html)

    section_html = section_html.replace('{{icons}}', '\n'.join(section_content) )
    section_html = section_html.replace('{{len}}', str(len(group['icons'])))
    section_html = section_html.replace('{{id}}', group['id'])
    ...
    nav += '<a href="#' + group['id'] + '" ' + '>' + group['name'] + '</a></li>'
    navs.append(nav)
    i += 1
  ...
  template_html = template_html.replace("{{page_navs}}", '\n'.join(navs))
