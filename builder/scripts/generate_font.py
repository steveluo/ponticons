...
for group in manifest_data['groups']:
    total_icons += len(group['icons'])

print "Load Manifest, Icons: %s" % ( total_icons )
...
for dirname, dirnames, filenames in os.walk(INPUT_SVG_DIR):
  ...
  fontfile = '%s/ponticons' % (OUTPUT_FONT_DIR)
...
if True:
  ...
  #manifest_data['icons'] = sorted(manifest_data['icons'], key=lambda k: k['code']) 
  #build_data['icons'] = sorted(build_data['icons'], key=lambda k: k['code']) 
